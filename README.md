...
Open CanvasToText.
Copy the full URL of the Canvas page containing the video.
Paste the URL into the Canvas URL field.
Click Fetch Manifest.
Microsoft Edge opens using CanvasToText's dedicated browser profile.
Sign into Canvas normally if requested.
Complete SSO or MFA normally if required.
Leave Edge open while CanvasToText searches the page.
When a supported native Canvas media manifest is found, the manifest field is populated automatically.
...
Open the Canvas page containing the video in your normal browser.
...
While the video is playing, search for requests containing terms such as:
media_attachments
redirect
.mpd
Locate the media manifest request associated with the video.
Open that request or URL.
...
Open CanvasToText.
Next to Local manifest, select:
Browse any file...
Select the saved manifest.
Choose the desired output settings.
Start transcription.
...
Install PyInstaller.
Install Inno Setup.
Extract the complete CanvasToText source package.
Run:
build_setup.bat
Wait for the application and installer to build.
...
One thing I’d specifically do after replacing the current README: rename the top-level GitHub description from **“CanvasToText v1.10”** to simply **“CanvasToText”** so you never have to keep the README version number synchronized with every release.
